    ...
    var titulo                      = "CARACTERISTICAS DEL INMUEBLE;";
    var INMUEBLES                   = null;
    var RUTA                        = "/REPMOVIL/"; //"/smallbox/"; //
    var RUTA_IMAGEN                 = "http://whalepanama.com/REP/modulos/inmuebles/imagenes/";  //"http://localhost/rep/modulos/inmuebles/imagenes/";

    var COD_INMUEBLE                = null;
    var INFORMACION_INMUEBLE        = null;
    var IMAGENES_INMUEBLE           = null;
    var CARACTERISTICAS_INMUEBLE    = null;
    var ALREDEDORES_INMUEBLE        = null;
    var UBICACION_INMUEBLE          = null;
    var LATITUD_INMUEBLE            = null;
    var LONGITUD_INMUEBLE           = null;
    var MAPA_INMUEBLE               = null;
    
    var URL                 = "http://whalepanama.com/REPMOVIL/common/ws_movil.php?format=json";  //"./common/ws_movil.php?format=json"; 
    var METODO              = null;
    var VALOR               = null;
    var LOADER              = null;
    var ERROR               = null;
...
$( document ).ready(function() {
    ...
    function init_template(){//Class is vital to run AJAX Pages
        ...
        if (window.location.pathname === RUTA + "index.html" || window.location.pathname === RUTA )
        {
            METODO  = "LEER_INMUEBLES"; 
            VALOR   = "COD_CAMPANIA";
            LOADER  = "#loader";
            ERROR   = "#error";
            ...
            call_Ajax_Jsonp(URL, METODO, VALOR, LOADER, ERROR, function(data) 
            {
                ...
                {
                    anuncio = 
                        '<a href="inmueble.html?id=' + data.INMUEBLES[index].COD_INMUEBLE + '" class="inmueble">' + 
                            '<div class="quote-2 container">' +
                                '<p class="quote-box">' + 
                                    '<img class="quote-image" src="' + RUTA_IMAGEN +  data.INMUEBLES[index].COD_INMUEBLE + '/1.jpg" alt="img">' +
                                    '<strong class="quote-author">' + 
                                           (data.INMUEBLES[index].NOMBRE).toUpperCase() + 
                                    '</strong>' +
                                '</p>' +
                                '<div class="clear"></div>' +
                                '<p class="quote-text">' +
                                        data.INMUEBLES[index].DESCRIPCION + 
                                '</p>' +
                                '<em class="quote-title">Distancia aproximada: ' + parseFloat(data.INMUEBLES[index].DISTANCIA).toFixed(0) + ' Kilometros</em>' +
                            '</div> ' + 
                        '</a>' + 
            ...
            $('a.inmueble').click(function(e) 
            { 
                COD_INMUEBLE = $(this).attr('id');
                localStorage.setItem("COD_INMUEBLE",COD_INMUEBLE);                
            });
        }
        else if (window.location.pathname === RUTA + "inmueble.html")
        {
            /*
           ...
           var qs = (function(a) 
           {
            ...
            METODO  = "LEER_INFORMACION_INMUEBLE"; 
            //VALOR   = location.search.split('id=')[1];
            VALOR   = qs["id"];
            LOADER  = "#loader";
            ERROR   = "#error";
            ...
            {   
                ...
                LATITUD_INMUEBLE            = INFORMACION_INMUEBLE.LATITUD;
                LONGITUD_INMUEBLE           = INFORMACION_INMUEBLE.LONGITUD;
                
                $("#NOMBRE_INMUEBLE").html((INFORMACION_INMUEBLE.NOMBRE).toUpperCase());
                $("#DESCRIPCION_INMUEBLE").html((INFORMACION_INMUEBLE.DESCRIPCION).toUpperCase());
                
                $.each(IMAGENES_INMUEBLE, function(index)
                {
                  imagen = 
                        '<a href=' + RUTA_IMAGEN + IMAGENES_INMUEBLE[index].COD_INMUEBLE + '/' + IMAGENES_INMUEBLE[index].NOMBRE + ' class="show-gallery" title="Your caption here!"><img src=' + RUTA_IMAGEN + IMAGENES_INMUEBLE[index].COD_INMUEBLE + '/' + IMAGENES_INMUEBLE[index].NOMBRE + ' alt="img"></a>';                 
                    $('#IMAGENES_INMUEBLE').append(imagen);
                });
        
                $.each(CARACTERISTICAS_INMUEBLE, function(index)
                {
                  caracteristica = 
                        '<a href="#" class="tasklist-item tasklist-green">' +
                            '<i class="fa fa-check"></i>' +
                            '<h5>' + CARACTERISTICAS_INMUEBLE[index].CARACTERISTICA + '</h5>' +
                        '</a>';                 
                    $('#CARACTERISTICAS_INMUEBLE').append(caracteristica);
                });
                ...
                $.each(ALREDEDORES_INMUEBLE, function(index)
                {
                  alrededor = 
                        '<a href="#" class="tasklist-item tasklist-green">' +
                            '<i class="fa fa-check"></i>' +
                            '<h5>' + ALREDEDORES_INMUEBLE[index].NOMBRE + '</h5>' +
                        '</a>';                 
                    $('#ALREDEDORES_INMUEBLE').append(alrededor);
                });
                
                mapa = 
                    '<iframe class="responsive-image maps" frameborder="0" style="border:0"  src="https://www.google.com/maps/embed/v1/place?q=' + INFORMACION_INMUEBLE.LATITUD + '%2C' + INFORMACION_INMUEBLE.LONGITUD + '&key=AIzaSyCF1c2rck-zoI5GXGEdJ9BuD9GqJzeWFmM" allowfullscreen></iframe>' +
                    '<a href="mapa.html" class="button button-red button-fullscreen uppercase ultrabold">VER EN PANTALLA COMPLETA <i class="fa fa-arrows-alt heading-icon"></i></a>';
                $('#MAPA').append(mapa);
        
                
            });
            /*
            $('a.show-gallery').click(function(e) 
            {
                return false;
            });*/
        }
        
        else if (window.location.pathname === RUTA + "mapa.html")
        {
            mapa = 
                '<iframe class="responsive-image maps" frameborder="0" style="border:0"  src="https://www.google.com/maps/embed/v1/place?q=' + INFORMACION_INMUEBLE.LATITUD + '%2C' + INFORMACION_INMUEBLE.LONGITUD + '&key=AIzaSyCF1c2rck-zoI5GXGEdJ9BuD9GqJzeWFmM" allowfullscreen></iframe>';
            $('#MAPA_FULL_SCREEN').append(mapa);
        }
